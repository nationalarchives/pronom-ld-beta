...
public class SearchResultDeserializer implements RDFDeserializer<SearchResult> {
    ...
    public SearchResult fromModel(Resource uri, Model model) {
        ...
        String match = mu.getOneObjectOrNull(uri, makeProp(PRONOM.SearchResult.Match)).asLiteral().getString();
        Resource field = mu.getOneObjectOrNull(uri, makeProp(PRONOM.SearchResult.Field)).asResource();
